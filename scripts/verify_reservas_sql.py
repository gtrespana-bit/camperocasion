#!/usr/bin/env python3
"""Comprueba las garantías de la reserva con señal (Fase 1.2, confirmación del
vendedor).

Ejecuta `setup-camperocasion.sql` en un Postgres de verdad (mismo arnés que
`validate_setup_sql.py`) y verifica:

  - Un anuncio NO queda bloqueado por una solicitud (estado `solicitada`): solo
    lo bloquea una reserva ACTIVA, confirmada por el vendedor.
  - Un anuncio no puede tener dos reservas activas a la vez.
  - El flag `productos.reservado` se propaga desde la reserva activa y se limpia
    al cancelar, expirar o vender.
  - Marcar el anuncio como vendido cierra solicitudes y reservas vivas.
...
# Segundo comprador: solicita tras cancelar la primera. Separado del "tercero"
...
R_ACTIVA = 'cccccccc-cccc-4ccc-8ccc-cccccccccccc'
...
cur.execute(
    """insert into public.reservas (id, producto_id, comprador_id, vendedor_id, importe, estado, expira_en)
       values (%s, %s, %s, %s, 300, 'activa', now() + interval '5 days')""",
    (R_ACTIVA, P_VENTA, U_COMPRADOR, U_VENDEDOR),
)
...
# ── Una reserva activa marca el anuncio como reservado ─────────────────────
check(
    contar("select reservado from public.productos where id=%s", (P_VENTA,)) is True,
    'la reserva activa marca el anuncio como reservado',
)
check(
...
# ── Una reserva activa por anuncio ─────────────────────────────────────────
err = intentar(
    """insert into public.reservas (producto_id, comprador_id, vendedor_id, importe, estado)
       values (%s,%s,%s,300,'activa')""",
    (P_VENTA, U_TERCERO, U_VENDEDOR),
)
check(err is not None and 'duplicate' in err.lower(), 'no se pueden tener dos reservas activas del mismo anuncio', (err or 'PERMITIDO')[:50])

# ── Una solicitud de OTRO no bloquea el anuncio ────────────────────────────
# (el índice único solo cubre solicitadas del MISMO (producto, comprador)).
err = intentar(
    """insert into public.reservas (producto_id, comprador_id, vendedor_id, importe, estado)
       values (%s,%s,%s,300,'solicitada')""",
    (P_VENTA, U_COMPRADOR2, U_VENDEDOR),
)
check(err is None, 'una solicitud de otro comprador no choca con la reserva activa', err or 'ok')
check(
    contar("select reservado from public.productos where id=%s", (P_VENTA,)) is True,
    'el anuncio sigue reservado porque hay una reserva activa',
)

# ── Cancelar la activa libera el anuncio ───────────────────────────────────
cur.execute("update public.reservas set estado='cancelada', actualizado_en=now() where id=%s", (R_ACTIVA,))
check(
    contar("select reservado from public.productos where id=%s", (P_VENTA,)) is False,
    'al cancelar la reserva activa, el anuncio vuelve a estar libre',
)
check(
...
# ── Una solicitud NO bloquea el anuncio ────────────────────────────────────
cur.execute(
    """update public.reservas set estado='solicitada', expira_en = now() + interval '5 days'
        where producto_id=%s and comprador_id=%s""",
    (P_VENTA, U_COMPRADOR2),
)
check(
    contar("select reservado from public.productos where id=%s", (P_VENTA,)) is False,
    'una solicitud pendiente NO bloquea el anuncio',
)

# ── Confirmar la solicitud bloquea ─────────────────────────────────────────
cur.execute(
    "update public.reservas set estado='activa', actualizado_en=now() where producto_id=%s and comprador_id=%s",
    (P_VENTA, U_COMPRADOR2),
)
check(
    contar("select reservado from public.productos where id=%s", (P_VENTA,)) is True,
    'confirmar la solicitud (activa) bloquea el anuncio',
)
...
# ── Vender cierra las solicitudes y reservas vivas ─────────────────────────
cur.execute(
...
como('authenticated', U_COMPRADOR)
err = intentar("update public.reservas set estado='activa' where id=%s", (R_ACTIVA,))
check(err is not None, 'un usuario no puede activarse su propia reserva', (err or 'PERMITIDO')[:50])
err = intentar("delete from public.reservas where id=%s", (R_ACTIVA,))
